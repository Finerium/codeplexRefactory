...
- [ ] Turn 1 test_gen (V4-Pro think high): generate failing tests, write under `drafts/add-2fa-to-login-67a53d/tests/`
- [ ] Turn 2 impl_gen (V4-Pro think high): generate implementation, write under `drafts/add-2fa-to-login-67a53d/`
- [ ] Turn 3 diff_serialize (V4-Flash non-think): produce `drafts/add-2fa-to-login-67a53d/diff.patch`
...
- [ ] On Accept: POST /api/refactor/add-2fa-to-login-67a53d/accept returns `drafts/add-2fa-to-login-67a53d/diff.patch` as a FileResponse download (OQ-09)
- [ ] On Discard: POST /api/refactor/add-2fa-to-login-67a53d/discard cleans up the drafts directory

## 4. Archival (post-Accept)

- [ ] Move `openspec/changes/add-2fa-to-login-67a53d/` to `openspec/archive/add-2fa-to-login-67a53d/`
- [ ] Mirror move under `.agent-openspec/archive/add-2fa-to-login-67a53d/`
- [ ] Update Demeter `proposals.stage` to `accepted`

## 5. Cleanup

- [ ] On Discard, mark Demeter `proposals.stage` to `discarded`
- [ ] Remove `drafts/add-2fa-to-login-67a53d/` (best-effort, see `drafts_isolation.cleanup_simulation_dir`)
