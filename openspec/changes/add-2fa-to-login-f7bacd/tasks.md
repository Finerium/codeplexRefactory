...
- [ ] Turn 1 test_gen (V4-Pro think high): generate failing tests, write under `drafts/add-2fa-to-login-f7bacd/tests/`
- [ ] Turn 2 impl_gen (V4-Pro think high): generate implementation, write under `drafts/add-2fa-to-login-f7bacd/`
- [ ] Turn 3 diff_serialize (V4-Flash non-think): produce `drafts/add-2fa-to-login-f7bacd/diff.patch`
...
- [ ] On Accept: POST /api/refactor/add-2fa-to-login-f7bacd/accept returns `drafts/add-2fa-to-login-f7bacd/diff.patch` as a FileResponse download (OQ-09)
- [ ] On Discard: POST /api/refactor/add-2fa-to-login-f7bacd/discard cleans up the drafts directory

## 4. Archival (post-Accept)

- [ ] Move `openspec/changes/add-2fa-to-login-f7bacd/` to `openspec/archive/add-2fa-to-login-f7bacd/`
- [ ] Mirror move under `.agent-openspec/archive/add-2fa-to-login-f7bacd/`
- [ ] Update Demeter `proposals.stage` to `accepted`

## 5. Cleanup

- [ ] On Discard, mark Demeter `proposals.stage` to `discarded`
- [ ] Remove `drafts/add-2fa-to-login-f7bacd/` (best-effort, see `drafts_isolation.cleanup_simulation_dir`)
