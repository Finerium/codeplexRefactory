/**
 * Marketing landing copy + glyphs + portraits.
 *
 * Authored by Calliope (Wave 1) ported 1-to-1 from Designer Prompt 1 bundle
 * file `app-data.jsx`. Voice preserved verbatim, dev-poetic declarative
 * fragments + specific numbers per Designer intent. Mode names + resident
 * names are LOCKED per PRD Section 10 and Designer Prompt 1 line 36, 44.
 */
import type { ReactNode } from 'react';

export interface ModeCopy {
  id: 'onboarding' | 'sprint' | 'refactor' | 'activity' | 'health';
  n: string;
  name: string;
  line: string;
  body: string;
...
export interface ResidentCopy {
  id: 'athena' | 'apollo' | 'argus' | 'clio' | 'hermes';
  name: string;
  role: string;
  landmark: string;
  line: string;
...
export const MODES: ModeCopy[] = [
  ...
  },
  {
    ...
    body: 'Six months of git history projected as light. Who lives where, which blocks were rebuilt this quarter, which corners have not been touched since the founding commit.',
    stat: '180 d',
    statLbl: 'commit memory',
  },
  {
    id: 'health',
    n: '05',
    ...
    stat: '47',
    statLbl: 'detectors live',
  },
];

export const RESIDENTS: ResidentCopy[] = [
  {
    id: 'athena',
    name: 'Athena',
    role: 'The Architect',
...
export const MODE_GLYPHS: Record<ModeCopy['id'], ReactNode> = {
  onboarding: (
...
export const RESIDENT_PORTRAITS: Record<ResidentCopy['id'], ReactNode> = {
  athena: (
