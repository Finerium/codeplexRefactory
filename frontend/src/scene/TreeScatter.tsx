...
import type { DistrictData } from './buildings';
 ...
 *   1. Per-district clusters (count by coverage)
 *   2. Inner plaza filler (PLAZA_TREE_COUNT)
 *   3. Outer ring belt (RING_TREE_COUNT)
 */
function collectTreePositions(districts: DistrictData[]): TreePosition[] {
  const positions: TreePosition[] = [];

  // Per-district clusters.
  for (const d of districts) {
    const coverage = mockCoverage(d.id);
    const count = densityFromCoverage(coverage);
    if (count === 0) continue;
...
export function TreeScatter() {
  ...
  const positions = useMemo(
    () => collectTreePositions(cityData.districts),
    [cityData.districts],
  );
